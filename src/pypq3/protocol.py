...
from .exceptions import KeyExchangeError, ProtocolStateError
...
class PQ3KeyExchange:
    ...
    def initiate_key_exchange(self, remote_identity: DeviceIdentity) -> Tuple[InitialKeyExchange, SharedSecret]:
        ...
        try:
            ...
            identity_shared, identity_kyber_ct = PQ3Crypto.perform_key_exchange_initiator(
            ...
            ephemeral_shared, ephemeral_kyber_ct = PQ3Crypto.perform_key_exchange_initiator(
                ephemeral_keypair,
                remote_identity.ecc_public_key,
                remote_identity.kyber_public_key
            )
            ...
            key_exchange = InitialKeyExchange(
                ...
                kyber_ciphertext=ephemeral_kyber_ct,  # Ephemeral Kyber ciphertext for Bob's identity key
    ...
    def respond_to_key_exchange(self, key_exchange: InitialKeyExchange) -> Tuple[InitialKeyExchange, SharedSecret]:
        """Respond to incoming key exchange."""
        try:
            # Verify sender identity (simplified - in real implementation would verify signatures)
            ...
            response = InitialKeyExchange(
                ...
                kyber_ciphertext=identity_kyber_ct,  # Send back kyber ciphertext from identity exchange
                timestamp=int(time.time())
            )
    ...
    def _combine_shared_secrets(self, identity_shared: SharedSecret, 
                              ephemeral_shared: SharedSecret) -> SharedSecret:
