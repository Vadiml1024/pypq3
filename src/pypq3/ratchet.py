...
class PQ3Ratchet:
    ...
    def __init__(self, initial_shared_secret: SharedSecret, 
                 is_alice: bool = True):
        ...
        if self.state.kyber_counter % self.KYBER_REKEY_INTERVAL == 0:
            try:
                ...
                kyber_shared = kyber.Kyber1024.decaps(
                    self.state.dh_keypair.kyber_private_key,
                    kyber_ciphertext
                )
                
                # Mix Kyber shared secret into root key
                self.state.root_key, _ = self._kdf_rk(
                    self.state.root_key, kyber_shared
                )
    ...
    def decrypt_message(self, header: bytes, ciphertext: bytes,
        ...
        try:
            ...
            if (self.state.dh_remote_public is None or 
                remote_public_key != self.state.dh_remote_public):
